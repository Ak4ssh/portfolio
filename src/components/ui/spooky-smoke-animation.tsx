...
void main(){
  ...
  // Mobile adaptive scaling: zoom out the camera on narrow screens
  if (R.x < R.y) {
      uv *= 2.5; 
  } else {
      uv.x += .25;
  ...
  // MASSIVE COLOR BOOST:
  // Instead of a weak mix, we extract the smoke intensity and forcefully tint 
  // the entire structure with pure u_color, boosting brightness by 1.8x!
  float smokeIntensity = dot(col, vec3(0.333));
  col = mix(col * 0.1, u_color * 1.8, smokeIntensity * 1.5);

  // Removed the slow fade-in delay so the smoke is instantly visible on page refresh
  col=clamp(col,.08,1.);
  O=vec4(col,1);
}`;
