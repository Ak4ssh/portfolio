...
export const PillBase: React.FC = () => {
  ...
  // Unique ID for unique SVG filter references
  ...
  return (
    <>
      {/* DISPLACEMENT SVG FILTER FOR NAVBAR */}
      <svg className="absolute w-0 h-0 overflow-hidden pointer-events-none" aria-hidden="true">
        <filter id={`nav-liquid-glass-${filterId}`} primitiveUnits="objectBoundingBox">
          <feImage 
            result="map" 
            width="100%" 
            height="100%" 
      ...
      {/* Custom styling for navbar refraction */}
      <style>{`
        .nav-liquid-lens {
          background-color: oklch(from var(--foreground) l c h / 8%);
          backdrop-filter: blur(14px) url(#nav-liquid-glass-${filterId}) saturate(160%);
          -webkit-backdrop-filter: blur(14px) saturate(160%);
          box-shadow: 
            inset 0 0 0 1px color-mix(in srgb, white 8%, transparent),
            inset 1.8px 3px 0px -2px color-mix(in srgb, white 80%, transparent), 
      ...
      <motion.nav
        ...
        className="relative rounded-full isolate select-none cursor-pointer"
        style={{
          width: pillWidth,
          height: '56px',
          x: pillShift,
      ...
      >
        {/* ISOLATED REFRACTION LENS LAYER */}
              ...
              <AnimatePresence mode="wait">
                {activeItem && (
                  <motion.span
                    ...
                    className="nav-text-shadow font-semibold tracking-wide text-white/90"
                    style={{
                      fontSize: '15px',
                      letterSpacing: '0.5px',
                      whiteSpace: 'nowrap',
              ...
              {navItems.map((item, index) => {
                ...
                return (
                  <motion.button
                    ...
                    className="relative cursor-pointer transition-all duration-200 select-none font-medium outline-none bg-transparent border-none nav-text-shadow"
                    style={{
                      fontSize: '15px',
